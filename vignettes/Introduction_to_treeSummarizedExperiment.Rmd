...
`SingleCellExperiment` class[@Aaron2019]. It's used to store rectangular data of
...
class. If the tree is availabel in other formats, one would need to convert it
...
with four additional lines about `rowLinks`, `rowTree`, `colLinks` and
`colTree`. Here, **rse** stores a row tree (`phylo` object), and the `rowLinks`
has `r nrow(rse)` rows that is exactly the same as the number of rows in the
`assays` tables. More details about the link data could be found in Section
\@ref(linkData).
...
It's exactly the same work as shown in `r Biocpkg("SingleCellExperiment")` to
exctract slots inherited from the `SingleCellExperiment` class.
...
as `r CRANpkg("ape")`[@ape2018], `r CRANpkg("tidytree")`. These functions might be useful
...
rTree <- rowTree(rte)
rLink <- rowLinks(rte)
treeLeaf <- printNode(tree = rTree,type = "leaf")$nodeNum
dataLeaf <- rLink$nodeNum[rLink$isLeaf]
rmLeaf <- setdiff(treeLeaf, dataLeaf)
ntse <- updateRowTree(tse = rte, dropLeaf = rmLeaf)
```
