...
Multiple `TreeSummarizedExperiemnt` objects (**TSE**) can be combined by using `rbind` or `cbind`. 
Here, we create a toy `TreeSummarizedExperiment` object using `makeTSE()`. As the tree in the row/column tree slot is generated randomly using `ape::rtree()`, `set.seed()` is used to create reproducible results.
